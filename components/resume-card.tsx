...
export const ResumeCard = ({
  ...
  return (
    <Card
      className="flex bg-background cursor-pointer overflow-hidden"
      onClick={handleClick}
    >
      ...
      <div className="flex-grow ml-4 items-center flex-col group">
        <CardHeader className="pb-2">
          ...
          {subtitle && (
            <div className="font-sans text-base sm:text-lg font-medium">
              {subtitle}
            </div>
          )}
          {description && (
            ...
            >
              {description.map((x) => (
                <p key={x} className="text-xs sm:text-sm mb-1 last:mb-0 mt-2">
                  {x}
                </p>
              ))}
            </motion.div>
